...
class DataEngine:
    def __init__(self, broker, strategies=None):
        ...
        self.fii_dii_data = {} # New: Store FII/DII data
        self.running = False
        self.thread = None
    ...
    def _poll_loop(self):
        while self.running:
            ...
            # 1. Update FII/DII (Mock) every 10 seconds for UI responsiveness
            if time.time() - self.fii_dii_last_update > 10:
                self._update_mock_fii_dii()
    ...
    def _update_mock_fii_dii(self):
        # Generate realistic-looking random data
        fii_net = random.randint(-5000, 5000)
        dii_net = random.randint(-3000, 3000)

        data = {
            "fii_net_crores": fii_net,
            "dii_net_crores": dii_net,
            "fii_prev_day": fii_net + random.randint(-500, 500),
            "dii_prev_day": dii_net + random.randint(-500, 500),
